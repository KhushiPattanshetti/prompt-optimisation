...
def set_training_loop(
    ...
    global _training_loop, _rollouts_dir, _seen_rollout_ids, _seen_rollout_ids_index, _traj_client
    _training_loop = loop
    _rollouts_dir = Path(rollouts_dir) if rollouts_dir is not None else None
    if _rollouts_dir is not None:
        _seen_rollout_ids_index = _rollouts_dir / ".seen_rollout_ids"
        _seen_rollout_ids = _load_seen_rollout_ids()

    trajectory_store_url = str(settings.trajectory_store_url or "").strip()
    if trajectory_store_url:
        _traj_client = TrajectoryStoreClient(
            base_url=trajectory_store_url,
            timeout=float(settings.trajectory_store_status_timeout_seconds),
            cache_ttl_seconds=float(settings.trajectory_store_healthcheck_ttl_seconds),
        )
    else:
        _traj_client = None
...
def _persist_rollout_batch(submissions: List[RolloutSubmission], run_id: Optional[str]) -> RolloutAck:
    ...
    with _rollout_lock:
        for submission in submissions:
            ...
            if rollout_id:
                dedup_key = f"{resolved_run_id}:{rollout_id}"
                if dedup_key in _seen_rollout_ids:
                    duplicate_count += 1
                    continue
                _seen_rollout_ids.add(dedup_key)
