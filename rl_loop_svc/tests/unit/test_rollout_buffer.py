...
class TestRolloutBuffer:
    def test_store_and_len(self):
        buf = RolloutBuffer()
        assert len(buf) == 0
        buf.store(0.5, -5.0, 0.3, "g1", "orig", "rewr")
        assert len(buf) == 1
    ...
    def test_returns_equals_advantage_plus_value(self):
        buf = RolloutBuffer()
        buf.store(0.5, -5.0, 0.4, "g1", "orig", "rewr")
    ...
    def test_build_prompts_preserved(self):
        buf = RolloutBuffer()
        buf.store(0.5, -5.0, 0.3, "g1", "clinical note A", "enhanced note A")
        buf.store(0.8, -4.0, 0.6, "g1", "clinical note B", "enhanced note B")
    ...
    def test_negative_rewards_stored_correctly(self):
        buf = RolloutBuffer()
        buf.store(-1.0, -10.0, -0.5, "g1", "orig", "rewr")
        assert len(buf) == 1
    ...
    def test_store_preserves_sample_weights(self):
        buf = RolloutBuffer()
        buf.store(0.5, -5.0, 0.3, "g1", "orig", "rewr", sample_weight=0.25)
