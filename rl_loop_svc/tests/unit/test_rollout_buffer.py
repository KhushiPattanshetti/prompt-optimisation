...
class TestRolloutBuffer:
    def test_store_and_len(self):
        buf = RolloutBuffer()
        assert len(buf) == 0
        buf.store(0.5, 0.5, -5.0, 0.3, "g1", "orig", "rewr")
        assert len(buf) == 1
    ...
    def test_build_shapes(self):
        buf = RolloutBuffer()
        for _ in range(4):
            buf.store(0.5, 0.4, -5.0, 0.3, "g1", "orig", "rewr")
        advantages = torch.zeros(4)
        batch = buf.build(advantages)
        assert batch.rewards.shape == (4,)
        assert batch.concept_rewards.shape == (4,)
        assert batch.sample_weights.shape == (4,)
    ...
    def test_returns_equals_advantage_plus_value(self):
        buf = RolloutBuffer()
        buf.store(0.5, 0.5, -5.0, 0.4, "g1", "orig", "rewr")
    ...
    def test_build_prompts_preserved(self):
        buf = RolloutBuffer()
        buf.store(0.5, 0.4, -5.0, 0.3, "g1", "clinical note A", "enhanced note A")
        buf.store(0.8, 0.6, -4.0, 0.6, "g1", "clinical note B", "enhanced note B")
        ...
        assert batch.group_ids == ["g1", "g1"]

    def test_negative_rewards_stored_correctly(self):
        buf = RolloutBuffer()
        buf.store(-1.0, -1.0, -10.0, -0.5, "g1", "orig", "rewr")
        assert len(buf) == 1
    ...
    def test_build_tensor_dtype(self):
        ...
        assert batch.concept_rewards.dtype == torch.float32
        assert batch.sample_weights.dtype == torch.float32
        assert batch.log_probs_old.dtype == torch.float32

    def test_store_preserves_sample_weights(self):
        buf = RolloutBuffer()
        buf.store(0.5, 0.5, -5.0, 0.3, "g1", "orig", "rewr", sample_weight=0.25)
        batch = buf.build(torch.zeros(1))
        assert batch.sample_weights.shape == (1,)
        assert abs(batch.sample_weights[0].item() - 0.25) < 1e-8
