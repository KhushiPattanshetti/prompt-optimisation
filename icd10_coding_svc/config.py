...
LOCAL_CACHE_PATH   = os.path.join(
  os.path.dirname(__file__), "..", "sft_checkpoints", "med42"
)
OUTPUT_PATH        = os.path.join(
  os.path.dirname(__file__), "..", "inference_outputs", "icd10"
)
GT_CODES_PATH      = os.path.join(
  os.path.dirname(__file__), "..", "gt_codes"
)

DATASET_SVC_URL    = os.environ.get("DATASET_SVC_URL",  "http://localhost:8003")
...
ICD_INPUT_MAX_LENGTH = int(os.environ.get("ICD_INPUT_MAX_LENGTH", "4096"))
ICD_PARSE_RECOVERY_ENABLED = os.environ.get("ICD_PARSE_RECOVERY_ENABLED", "1") == "1"
ICD_PARSE_RECOVERY_MAX_NOTE_CHARS = int(
  os.environ.get("ICD_PARSE_RECOVERY_MAX_NOTE_CHARS", "12000")
)
